    ...
    <div>
      ...
      <div class="task-info-style task-info-padding">
        ...
        <div class="task-info-description task-info-style">
          <div class="result-padding" style="padding-top:9px;">
            上次结果：
          </div>
          <div>
            <div class="task-info-style task-info-padding">
...
<script>
  import {task_add_interfaces,task_delete_interfaces,task_get_interfaces} from "../../requests/task";
  import select_interface_dialog from "./select_interfaces"
  import task_results from "./task_result_list"

  export default {
    name: "task_detail",
    props:['task'],  //任务的详细数据id,name,description等
    components:{
      ...
      get_task_interface_list(){
        task_get_interfaces(this.task.id).then(data=>{
          if(true === data.success){
            ...
            this.$message.error("获取任务接口失败")
          }
        })
      },
      open_edit_interface(data){
        window.open('/edit/interface?service=' + data.service + '&interface_id=' + data.id);
      },
      open_remove_interface(id){
        task_delete_interfaces(this.task.id,{task_interface_id:id}).then(data=>{
          if(true === data.success){
            this.get_task_interface_list()
          }else{
