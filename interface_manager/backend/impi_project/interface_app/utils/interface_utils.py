...
class InterfaceUtils:
    ...
    def parse_parameter(cls,parameter):
        """form形式的参数转成字典，例如： [{'key': '111', 'value': '1', 'type': 'string'}, {'key': '222', 'value': '2',
        'type': 'bool'}, {'key': '333', 'value': '3', 'type': 'float'}] 会转成字典{"a":"a","b":1}"""
        ret = {}
        if not parameter:
            return ret
    ...
    def send_request(cls,url,method,header,parameter,parameter_type):
        ret = ''
        if "form" == parameter_type: #第一步，把form形式转成字典，如果是json则不需要
