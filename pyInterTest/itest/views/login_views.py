...
from itest.util.tools import json_to_dict
from itest.util.tools import get_request_key
...
def register(request):
    ...
    if request.method == "POST":
        ...
        ret = json_to_dict(request.body)
        user_name = get_request_key(ret,"user")
        pwd = get_request_key(ret,"pwd")
        ...
        if user is None:
            ...
            if new_user is None:
                return tools.response_failed("创建账号失败")
            new_token = TokenDao.create_token(new_user, token, date)
            if new_token is None:
                return tools.response_failed("token创建失败")

            cache.set(token, new_token.expireDate, settings.REDIS_TIMEOUT)

            resp_dict = {"token": token, "token_name": globalVars.IToken, "token_expird": 30}
            return tools.response_by_json(
                data=resp_dict)
        else:
            return tools.response_failed("账号已存在，请重新输入！")

    # 访问登录页面
    else:
        return render(request, "register.html", tools.response_by_json(message="访问注册页面成功"))
